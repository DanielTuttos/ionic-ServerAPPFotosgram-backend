...
postRoutes.get('/', async (req: any, res: Response) => {
    ...
    const posts = await Post.find().skip(skip).sort({ _id: -1 }).limit(10).populate('usuario', '-password').exec();
...
postRoutes.post('/', [verificaToken], (req: any, res: Response) => {
    ...
    Post.create(body).then(async postDB => {
        ...
        await postDB.populate('usuario', '-password').execPopulate();//trea toda la informacion del elemento relacionado
